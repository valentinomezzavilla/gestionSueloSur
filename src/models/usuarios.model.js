...
const UsuariosModel = {
  ...
  listar() {
    return db.prepare(`
      SELECT id, usuario, nombre, rol, activo, created_at FROM users ORDER BY nombre
    `).all()
  },

  obtener(id) {
    return db.prepare(`
      SELECT id, usuario, nombre, rol, activo FROM users WHERE id = ?
    `).get(id)
  },
  ...
  actualizar(id, { usuario, nombre, rol, password }) {
    if (password) {
      const hash = bcrypt.hashSync(password, 10)
      db.prepare(`
        UPDATE users SET usuario = ?, nombre = ?, rol = ?, password_hash = ? WHERE id = ?
      `).run(usuario, nombre, rol, hash, id)
    } else {
      db.prepare(`
        UPDATE users SET usuario = ?, nombre = ?, rol = ? WHERE id = ?
  ...
  existeUsuario(usuario, excludeId = null) {
    if (excludeId) {
      return db.prepare(`SELECT 1 FROM users WHERE usuario = ? AND id != ?`).get(usuario, excludeId)
    }
