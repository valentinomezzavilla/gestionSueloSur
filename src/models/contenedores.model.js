...
// Sub-query reutilizable: trae el último movimiento por contenedor.
// Desempata por rowid (orden de inserción) cuando dos movimientos tienen la misma fecha.
...
const ContenedoresModel = {
  ...
  obtener(id) {
    ...
    c.movimientos = db.prepare(`
      ...
      LEFT JOIN op_encabezado op ON op.id = oc.id_orden_pedido
      LEFT JOIN clientes cli ON cli.id = op.id_cliente
      WHERE m.id_contenedor = ?
      ORDER BY m.fecha_movimiento DESC, m.rowid DESC
    `).all(id)
    return c
  },
  ...
  estadoActual(id) {
    return db.prepare(`
      SELECT * FROM movimiento_contenedor
      WHERE id_contenedor = ?
  ...
  crear({ numero_contenedor, estado_general, fecha_ultima_pintada, observaciones }) {
    ...
    db.transaction(() => {
      db.prepare(`
        INSERT INTO contenedores (id, numero_contenedor, estado_general, fecha_ultima_pintada, observaciones)
        VALUES (?, ?, ?, ?, ?)
      `).run(id, numero_contenedor, estado_general || 'operativo',
             fecha_ultima_pintada || null, observaciones || '')
      db.prepare(`
  ...
  actualizar(id, { numero_contenedor, estado_general, fecha_ultima_pintada, observaciones }) {
    db.prepare(`
      UPDATE contenedores
      SET numero_contenedor = ?, estado_general = ?, fecha_ultima_pintada = ?, observaciones = ?
      WHERE id = ?
    `).run(numero_contenedor, estado_general || 'operativo',
           fecha_ultima_pintada || null, observaciones || '', id)
  },
  ...
  registrarMovimiento({ id_contenedor, id_op_contenedor, id_chofer, id_camion, estado_paso, observaciones, fecha_movimiento }) {
    db.prepare(`
      INSERT INTO movimiento_contenedor
        (id, id_contenedor, id_op_contenedor, id_chofer, id_camion, fecha_movimiento, estado_paso, observaciones)
      VALUES (?, ?, ?, ?, ?, COALESCE(?, datetime('now')), ?, ?)
    `).run(
      crypto.randomUUID(), id_contenedor,
      id_op_contenedor || null, id_chofer || null, id_camion || null,
      fecha_movimiento || null, estado_paso, observaciones || ''
    )
  },

  // ── Circuito diario: contenedores a retirar ───────────────────
  // Un contenedor está "a retirar" cuando:
  //   • su último movimiento es 'entregado' o 'en_alquiler'
  //   • los días en domicilio >= plazo_alquiler de su OP activa
  // Se agrupan por zona para armar la hoja de ruta del día.
  circuitoDiario() {
    return db.prepare(`
  ...
  // ── Auxiliares para asignar contenedores a OPs ────────────────
  disponibles() {
    return db.prepare(`
      SELECT c.id, c.numero_contenedor
      FROM contenedores c
  ...
  resumenPorEstado() {
    return db.prepare(`
      SELECT um.estado_paso, COUNT(*) AS total
      FROM contenedores c
      JOIN (${SQL_ULTIMO_MOV}) um ON um.id_contenedor = c.id
      WHERE c.activo = 1
      GROUP BY um.estado_paso
    `).all()
  },

  // ── Asignar un contenedor a una OP existente ──────────────────
  asignarAContenedorOP(id_op_contenedor, id_contenedor) {
    db.prepare(`UPDATE op_detalle_contenedor SET id_contenedor = ? WHERE id = ?`)
      .run(id_contenedor, id_op_contenedor)
  },

  // ── Auxiliares de flota / choferes para los selects ───────────
  choferes() {
    return db.prepare(`
      SELECT id, nombre FROM users WHERE rol = 'chofer' AND activo = 1 ORDER BY nombre
    `).all()
  },

  camiones() {
    return db.prepare(`
      SELECT id, patente, nombre FROM flota_vehiculos
      WHERE tipo_vehiculo = 'camion' AND activo = 1 ORDER BY nombre
    `).all()
  },
}
