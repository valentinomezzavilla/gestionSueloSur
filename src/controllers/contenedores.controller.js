...
const ContenedoresController = {
  ...
  crear(req, res) {
    try {
      ...
      if (!numero || numero <= 0) {
        req.flash('error', 'El número de contenedor es obligatorio.')
        return res.redirect('/contenedores/nuevo')
      }
      if (ContenedoresModel.obtenerPorNumero(numero)) {
        req.flash('error', `Ya existe un contenedor con el número ${numero}.`)
        return res.redirect('/contenedores/nuevo')
      }
      ContenedoresModel.crear({
        numero_contenedor:    numero,
        estado_general:       req.body.estado_general,
        fecha_ultima_pintada: req.body.fecha_ultima_pintada,
        observaciones:        req.body.observaciones,
      })
      req.flash('success', `Contenedor N° ${numero} creado correctamente.`)
  ...
  actualizar(req, res) {
    try {
      ...
      if (existente && existente.id !== req.params.id) {
        req.flash('error', `Ya existe un contenedor con el número ${numero}.`)
        return res.redirect(`/contenedores/${req.params.id}/editar`)
      }
      ContenedoresModel.actualizar(req.params.id, {
        numero_contenedor:    numero,
        estado_general:       req.body.estado_general,
        fecha_ultima_pintada: req.body.fecha_ultima_pintada,
        observaciones:        req.body.observaciones,
      })
      req.flash('success', 'Contenedor actualizado correctamente.')
      res.redirect('/contenedores')
    } catch (err) {
      console.error(err)
      req.flash('error', 'Error al actualizar el contenedor.')
      res.redirect('/contenedores')
    }
  },
  ...
  toggleActivo(req, res) {
    try {
      ...
      req.flash('success', 'Estado del contenedor actualizado.')
    } catch (err) {
      console.error(err)
  ...
  detalle(req, res) {
    try {
      ...
      res.render('pages/contenedores/detalle', {
        titulo: `Contenedor N° ${contenedor.numero_contenedor}`,
        contenedor,
        choferes,
        camiones,
      })
  ...
  registrarMovimiento(req, res) {
    try {
      ...
      if (!estado_paso) {
        req.flash('error', 'Debe indicar el nuevo estado del contenedor.')
        return res.redirect(`/contenedores/${req.params.id}`)
      }
      ContenedoresModel.registrarMovimiento({
        id_contenedor:    req.params.id,
        estado_paso,
        id_chofer:        id_chofer || null,
        id_camion:        id_camion || null,
        observaciones,
  ...
  circuito(req, res) {
    try {
      ...
      res.render('pages/contenedores/circuito', {
        titulo: 'Circuito del Día',
        porZona,
        total: items.length,
      })
    } catch (err) {
      console.error(err)
      req.flash('error', 'Error al armar el circuito diario.')
      res.redirect('/contenedores')
    }
  },
}
